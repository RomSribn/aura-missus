...
- **Проверка личности при запросе по почте — решено: вариант C** (`117`).
  Оператор проверяет два факта из трёх по процедуре `116` и удаляет через
  CLI. Журнал удалений сервис пишет сам (`AURAT-0073`). Текст §02 страницы
  удаления этому соответствует, разработки нет.
