...
- **`CFBundleDisplayName` на iOS до сих пор `PsychoApp`**, тогда как Android
  показывает `Aura` (`strings.xml`). То есть под иконкой на iPhone будет стоять
  «PsychoApp». Это одна строка в `Info.plist`, но это **видимое пользователю
  имя**, а решение владельца касалось идентификатора, а не названия. Стоит
  закрыть до первой сборки в TestFlight.
